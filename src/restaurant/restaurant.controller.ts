...
export class RestaurantController {
    ...
    @Get()
    findAll() {
        this.log.debug('REST request to execute find Restaurant')
    ...
    async findById(@Param('id', ParseIntPipe) id:number) {
        ...
        if(! await this.restaurantRepository.existsBy({id:id})) {
            this.log.debug(`REST request not found with id: ${id}`);
    ...
    async create(@Body() restaurant: Restaurant) {
        ...
        try {
            ...
            this.log.error(`Error intentando crear un nuevo restaurante ${restaurant}}`);
